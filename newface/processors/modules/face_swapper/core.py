...
import newface.jobs.job_manager
import newface.jobs.job_store
from newface import config, content_analyser, face_classifier, face_detector, face_landmarker, face_masker, face_recognizer, inference_manager, logger, state_manager, translator, video_manager
from newface.common_helper import get_first, is_macos
from newface.download import conditional_download_hashes, conditional_download_sources, resolve_download_url
from newface.execution import has_execution_provider
from newface.face_analyser import get_average_face, get_many_faces, get_one_face, scale_face
from newface.face_helper import paste_back, warp_face_by_face_landmark_5
from newface.face_masker import create_area_mask, create_box_mask, create_occlusion_mask, create_region_mask
from newface.face_selector import select_faces, sort_faces_by_order
from newface.filesystem import filter_image_paths, has_image, in_directory, is_image, is_video, resolve_relative_path, same_file_extension
from newface.model_helper import get_static_model_initializer
from newface.processors.modules.face_swapper import choices as face_swapper_choices
from newface.processors.modules.face_swapper.types import FaceSwapperInputs
from newface.processors.pixel_boost import explode_pixel_boost, implode_pixel_boost
from newface.processors.types import ProcessorOutputs
from newface.program_helper import find_argument_group
from newface.thread_helper import conditional_thread_semaphore
from newface.types import ApplyStateItem, Args, DownloadScope, Embedding, Face, InferencePool, ModelOptions, ModelSet, ProcessMode, VisionFrame
from newface.vision import read_static_image, read_static_images, read_static_video_frame, unpack_resolution
...
def register_args(program : ArgumentParser) -> None:
	...
	if group_processors:
		...
		newface.jobs.job_store.register_step_keys([ 'face_swapper_model', 'face_swapper_pixel_boost', 'face_swapper_weight' ])
...
def forward_swap_face(source_face : Face, target_face : Face, crop_vision_frame : VisionFrame) -> VisionFrame:
	...
	if is_macos() and has_execution_provider('coreml') and model_type in [ 'ghost', 'uniface' ]:
		face_swapper.set_providers([ newface.choices.execution_provider_set.get('cpu') ])
