...
from newface.types import BenchmarkMode, BenchmarkResolution
...
def render() -> None:
	...
	BENCHMARK_MODE_DROPDOWN = gradio.Dropdown(
		...
		choices = newface.choices.benchmark_modes,
	...
	BENCHMARK_RESOLUTIONS_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = newface.choices.benchmark_resolutions,
	...
	BENCHMARK_CYCLE_COUNT_SLIDER = gradio.Slider(
		...
		step = calculate_int_step(newface.choices.benchmark_cycle_count_range),
		minimum = newface.choices.benchmark_cycle_count_range[0],
		maximum = newface.choices.benchmark_cycle_count_range[-1]
	)
