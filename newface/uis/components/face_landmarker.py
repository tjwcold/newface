...
from newface import face_landmarker, state_manager, translator
from newface.common_helper import calculate_float_step
from newface.types import FaceLandmarkerModel, Score
...
def render() -> None:
	...
	FACE_LANDMARKER_MODEL_DROPDOWN = gradio.Dropdown(
		...
		choices = newface.choices.face_landmarker_models,
	...
	FACE_LANDMARKER_SCORE_SLIDER = gradio.Slider(
		...
		step = calculate_float_step(newface.choices.face_landmarker_score_range),
		minimum = newface.choices.face_landmarker_score_range[0],
		maximum = newface.choices.face_landmarker_score_range[-1]
	)
