...
from newface.face_analyser import get_many_faces
from newface.face_selector import sort_and_filter_faces
from newface.face_store import clear_static_faces
from newface.filesystem import is_image, is_video
from newface.types import FaceSelectorMode, FaceSelectorOrder, Gender, Race, VisionFrame
from newface.uis.core import get_ui_component, get_ui_components, register_ui_component
from newface.uis.types import ComponentOptions
from newface.uis.ui_helper import convert_str_none
from newface.vision import fit_cover_frame, read_static_image, read_video_frame
...
def render() -> None:
	...
	FACE_SELECTOR_MODE_DROPDOWN = gradio.Dropdown(
		...
		choices = newface.choices.face_selector_modes,
	...
	with gradio.Group():
		with gradio.Row():
			FACE_SELECTOR_ORDER_DROPDOWN = gradio.Dropdown(
				...
				choices = newface.choices.face_selector_orders,
			...
			FACE_SELECTOR_GENDER_DROPDOWN = gradio.Dropdown(
				...
				choices = [ 'none' ] + newface.choices.face_selector_genders,
			...
			FACE_SELECTOR_RACE_DROPDOWN = gradio.Dropdown(
				...
				choices = [ 'none' ] + newface.choices.face_selector_races,
				value = state_manager.get_item('face_selector_race') or 'none'
			)
		with gradio.Row():
			face_selector_age_start = state_manager.get_item('face_selector_age_start') or newface.choices.face_selector_age_range[0]
			face_selector_age_end = state_manager.get_item('face_selector_age_end') or newface.choices.face_selector_age_range[-1]
			FACE_SELECTOR_AGE_RANGE_SLIDER = RangeSlider(
				label = translator.get('uis.face_selector_age_range_slider'),
				minimum = newface.choices.face_selector_age_range[0],
				maximum = newface.choices.face_selector_age_range[-1],
				value = (face_selector_age_start, face_selector_age_end),
				step = calculate_int_step(newface.choices.face_selector_age_range)
			)
	REFERENCE_FACE_DISTANCE_SLIDER = gradio.Slider(
		label = translator.get('uis.reference_face_distance_slider'),
		value = state_manager.get_item('reference_face_distance'),
		step = calculate_float_step(newface.choices.reference_face_distance_range),
		minimum = newface.choices.reference_face_distance_range[0],
		maximum = newface.choices.reference_face_distance_range[-1],
		visible = 'reference' in state_manager.get_item('face_selector_mode')
	)
