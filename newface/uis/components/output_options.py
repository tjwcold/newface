...
from newface.filesystem import is_image, is_video
from newface.types import AudioEncoder, Fps, Scale, VideoEncoder, VideoPreset
from newface.uis.core import get_ui_components, register_ui_component
from newface.vision import detect_video_fps
...
def render() -> None:
	...
	OUTPUT_IMAGE_QUALITY_SLIDER = gradio.Slider(
		...
		step = calculate_int_step(newface.choices.output_image_quality_range),
		minimum = newface.choices.output_image_quality_range[0],
		maximum = newface.choices.output_image_quality_range[-1],
	...
	OUTPUT_IMAGE_SCALE_SLIDER = gradio.Slider(
		...
		step = calculate_float_step(newface.choices.output_image_scale_range),
		value = state_manager.get_item('output_image_scale'),
		minimum = newface.choices.output_image_scale_range[0],
		maximum = newface.choices.output_image_scale_range[-1],
		visible = is_image(state_manager.get_item('target_path'))
	)
	...
	OUTPUT_AUDIO_QUALITY_SLIDER = gradio.Slider(
		...
		step = calculate_int_step(newface.choices.output_audio_quality_range),
		minimum = newface.choices.output_audio_quality_range[0],
		maximum = newface.choices.output_audio_quality_range[-1],
		visible = is_video(state_manager.get_item('target_path'))
	)
	OUTPUT_AUDIO_VOLUME_SLIDER = gradio.Slider(
		...
		step = calculate_int_step(newface.choices.output_audio_volume_range),
		minimum = newface.choices.output_audio_volume_range[0],
		maximum = newface.choices.output_audio_volume_range[-1],
		visible = is_video(state_manager.get_item('target_path'))
	)
	...
	OUTPUT_VIDEO_PRESET_DROPDOWN = gradio.Dropdown(
		...
		choices = newface.choices.output_video_presets,
	...
	OUTPUT_VIDEO_QUALITY_SLIDER = gradio.Slider(
		...
		step = calculate_int_step(newface.choices.output_video_quality_range),
		minimum = newface.choices.output_video_quality_range[0],
		maximum = newface.choices.output_video_quality_range[-1],
		visible = is_video(state_manager.get_item('target_path'))
	)
	OUTPUT_VIDEO_SCALE_SLIDER = gradio.Slider(
		label = translator.get('uis.output_video_scale_slider'),
		step = calculate_float_step(newface.choices.output_video_scale_range),
		value = state_manager.get_item('output_video_scale'),
		minimum = newface.choices.output_video_scale_range[0],
		maximum = newface.choices.output_video_scale_range[-1],
		visible = is_video(state_manager.get_item('target_path'))
	)
