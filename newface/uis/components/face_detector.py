...
from newface import face_detector, state_manager, translator
from newface.common_helper import calculate_float_step, get_last
from newface.sanitizer import sanitize_int_range
from newface.types import Angle, FaceDetectorModel, Score
...
def render() -> None:
	...
	if state_manager.get_item('face_detector_size') in newface.choices.face_detector_set[state_manager.get_item('face_detector_model')]:
		face_detector_size_dropdown_options['choices'] = newface.choices.face_detector_set[state_manager.get_item('face_detector_model')]
	with gradio.Row():
		FACE_DETECTOR_MODEL_DROPDOWN = gradio.Dropdown(
			label = translator.get('uis.face_detector_model_dropdown'),
			choices = newface.choices.face_detector_models,
	...
	FACE_DETECTOR_MARGIN_SLIDER = gradio.Slider(
		...
		step = calculate_float_step(newface.choices.face_detector_margin_range),
		minimum = newface.choices.face_detector_margin_range[0],
		maximum = newface.choices.face_detector_margin_range[-1]
	)
	FACE_DETECTOR_ANGLES_CHECKBOX_GROUP = gradio.CheckboxGroup(
		label = translator.get('uis.face_detector_angles_checkbox_group'),
		choices = newface.choices.face_detector_angles,
	...
	FACE_DETECTOR_SCORE_SLIDER = gradio.Slider(
		...
		step = calculate_float_step(newface.choices.face_detector_score_range),
		minimum = newface.choices.face_detector_score_range[0],
		maximum = newface.choices.face_detector_score_range[-1]
	)
...
def update_face_detector_model(face_detector_model : FaceDetectorModel) -> Tuple[gradio.Dropdown, gradio.Dropdown]:
	...
	if face_detector.pre_check():
		face_detector_size_choices = newface.choices.face_detector_set.get(state_manager.get_item('face_detector_model'))
...
def update_face_detector_margin(face_detector_margin : int) -> None:
	face_detector_margin = sanitize_int_range(face_detector_margin, newface.choices.face_detector_margin_range)
...
def update_face_detector_angles(face_detector_angles : Sequence[Angle]) -> gradio.CheckboxGroup:
	face_detector_angles = face_detector_angles or newface.choices.face_detector_angles
