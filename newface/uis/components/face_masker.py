...
from newface import face_masker, state_manager, translator
from newface.common_helper import calculate_float_step, calculate_int_step
from newface.sanitizer import sanitize_int_range
from newface.types import FaceMaskArea, FaceMaskRegion, FaceMaskType, FaceOccluderModel, FaceParserModel
from newface.uis.core import register_ui_component
...
def render() -> None:
	...
	with gradio.Row():
		FACE_OCCLUDER_MODEL_DROPDOWN = gradio.Dropdown(
			...
			choices = newface.choices.face_occluder_models,
		...
		FACE_PARSER_MODEL_DROPDOWN = gradio.Dropdown(
			...
			choices = newface.choices.face_parser_models,
	...
	FACE_MASK_TYPES_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = newface.choices.face_mask_types,
	...
	FACE_MASK_AREAS_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = newface.choices.face_mask_areas,
	...
	FACE_MASK_REGIONS_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = newface.choices.face_mask_regions,
	...
	FACE_MASK_BLUR_SLIDER = gradio.Slider(
		...
		step = calculate_float_step(newface.choices.face_mask_blur_range),
		minimum = newface.choices.face_mask_blur_range[0],
		maximum = newface.choices.face_mask_blur_range[-1],
	...
	with gradio.Group(visible = has_box_mask) as FACE_MASK_BOX_WRAPPER:
		with gradio.Row():
			FACE_MASK_PADDING_TOP_SLIDER = gradio.Slider(
				...
				step = calculate_int_step(newface.choices.face_mask_padding_range),
				minimum = newface.choices.face_mask_padding_range[0],
				maximum = newface.choices.face_mask_padding_range[-1],
...
def update_face_mask_types(face_mask_types : List[FaceMaskType]) -> Tuple[gradio.CheckboxGroup, gradio.CheckboxGroup, gradio.CheckboxGroup, gradio.Slider, gradio.Group]:
	face_mask_types = face_mask_types or newface.choices.face_mask_types
...
def update_face_mask_areas(face_mask_areas : List[FaceMaskArea]) -> gradio.CheckboxGroup:
	face_mask_areas = face_mask_areas or newface.choices.face_mask_areas
...
def update_face_mask_regions(face_mask_regions : List[FaceMaskRegion]) -> gradio.CheckboxGroup:
	face_mask_regions = face_mask_regions or newface.choices.face_mask_regions
...
def update_face_mask_padding(face_mask_padding_top : float, face_mask_padding_right : float, face_mask_padding_bottom : float, face_mask_padding_left : float) -> None:
	face_mask_padding_top = sanitize_int_range(int(face_mask_padding_top), newface.choices.face_mask_padding_range)
	face_mask_padding_right = sanitize_int_range(int(face_mask_padding_right), newface.choices.face_mask_padding_range)
	face_mask_padding_bottom = sanitize_int_range(int(face_mask_padding_bottom), newface.choices.face_mask_padding_range)
	face_mask_padding_left = sanitize_int_range(int(face_mask_padding_left), newface.choices.face_mask_padding_range)
