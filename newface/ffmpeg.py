...
from newface import ffmpeg_builder, logger, process_manager, state_manager, translator
from newface.filesystem import get_file_format, remove_file
from newface.temp_helper import get_temp_file_path, get_temp_frames_pattern
from newface.types import AudioBuffer, AudioEncoder, Command, EncoderSet, Fps, Resolution, UpdateProgress, VideoEncoder, VideoFormat
from newface.vision import detect_video_duration, detect_video_fps, pack_resolution, predict_video_frame_total
...
def get_available_encoder_set() -> EncoderSet:
	...
	while line := process.stdout.readline().decode().lower():
		if line.startswith(' a'):
			...
			if audio_encoder in newface.choices.output_audio_encoders:
				index = newface.choices.output_audio_encoders.index(audio_encoder) #type:ignore[arg-type]
				available_encoder_set['audio'].insert(index, audio_encoder) #type:ignore[arg-type]
		if line.startswith(' v'):
			video_encoder = line.split()[1]

			if video_encoder in newface.choices.output_video_encoders:
				index = newface.choices.output_video_encoders.index(video_encoder) #type:ignore[arg-type]
