...
from newface.filesystem import create_directory, is_directory
from newface.types import ExecutionDevice, ExecutionProvider, InferenceOptionSet, InferenceProvider, ValueAndUnit
...
def get_available_execution_providers() -> List[ExecutionProvider]:
	...
	for execution_provider, execution_provider_value in newface.choices.execution_provider_set.items():
		if execution_provider_value in inference_session_providers:
			index = newface.choices.execution_providers.index(execution_provider)
...
def create_inference_providers(execution_device_id : int, execution_providers : List[ExecutionProvider]) -> List[InferenceProvider]:
	...
	for execution_provider in execution_providers:
		if execution_provider == 'cuda':
			inference_providers.append((newface.choices.execution_provider_set.get(execution_provider),
			{
		...
		if execution_provider == 'tensorrt':
			...
			inference_providers.append((newface.choices.execution_provider_set.get(execution_provider), inference_option_set))
	...
	if 'cpu' in execution_providers:
		inference_providers.append(newface.choices.execution_provider_set.get('cpu'))
