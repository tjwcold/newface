...
from newface import inference_manager, state_manager
from newface.download import conditional_download_hashes, conditional_download_sources, resolve_download_url
from newface.filesystem import resolve_relative_path
from newface.thread_helper import conditional_thread_semaphore
from newface.types import DownloadScope, DownloadSet, FaceLandmark68, FaceMaskArea, FaceMaskRegion, InferencePool, Mask, ModelSet, Padding, VisionFrame
...
def create_area_mask(crop_vision_frame : VisionFrame, face_landmark_68 : FaceLandmark68, face_mask_areas : List[FaceMaskArea]) -> Mask:
	...
	for face_mask_area in face_mask_areas:
		if face_mask_area in newface.choices.face_mask_area_set:
			landmark_points.extend(newface.choices.face_mask_area_set.get(face_mask_area))
...
def create_region_mask(crop_vision_frame : VisionFrame, face_mask_regions : List[FaceMaskRegion]) -> Mask:
	...
	region_mask = numpy.isin(region_mask.argmax(0), [ newface.choices.face_mask_region_set.get(face_mask_region) for face_mask_region in face_mask_regions ])
