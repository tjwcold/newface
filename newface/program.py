...
from newface import config, metadata, state_manager, translator
from newface.common_helper import create_float_metavar, create_int_metavar, get_first, get_last
from newface.execution import get_available_execution_providers
from newface.ffmpeg import get_available_encoder_set
from newface.filesystem import get_file_name, resolve_file_paths
from newface.jobs import job_store
from newface.processors.core import get_processors_modules
from newface.sanitizer import sanitize_int_range, sanitize_job_id
...
def create_config_path_program() -> ArgumentParser:
	...
	group_paths.add_argument('--config-path', help = translator.get('help.config_path'), default = 'newface.ini')
...
def create_face_detector_program() -> ArgumentParser:
	...
	group_face_detector.add_argument('--face-detector-model', help = translator.get('help.face_detector_model'), default = config.get_str_value('face_detector', 'face_detector_model', 'yolo_face'), choices = newface.choices.face_detector_models)
	known_args, _ = program.parse_known_args()
	face_detector_size_choices = newface.choices.face_detector_set.get(known_args.face_detector_model)
	...
	group_face_detector.add_argument('--face-detector-margin', help = translator.get('help.face_detector_margin'), type = partial(sanitize_int_range, int_range = newface.choices.face_detector_margin_range), default = config.get_int_list('face_detector', 'face_detector_margin', '0 0 0 0'), nargs = '+')
	group_face_detector.add_argument('--face-detector-angles', help = translator.get('help.face_detector_angles'), type = int, default = config.get_int_list('face_detector', 'face_detector_angles', '0'), choices = newface.choices.face_detector_angles, nargs = '+', metavar = 'FACE_DETECTOR_ANGLES')
	group_face_detector.add_argument('--face-detector-score', help = translator.get('help.face_detector_score'), type = float, default = config.get_float_value('face_detector', 'face_detector_score', '0.5'), choices = newface.choices.face_detector_score_range, metavar = create_float_metavar(newface.choices.face_detector_score_range))
...
def create_face_landmarker_program() -> ArgumentParser:
	...
	group_face_landmarker.add_argument('--face-landmarker-model', help = translator.get('help.face_landmarker_model'), default = config.get_str_value('face_landmarker', 'face_landmarker_model', '2dfan4'), choices = newface.choices.face_landmarker_models)
	group_face_landmarker.add_argument('--face-landmarker-score', help = translator.get('help.face_landmarker_score'), type = float, default = config.get_float_value('face_landmarker', 'face_landmarker_score', '0.5'), choices = newface.choices.face_landmarker_score_range, metavar = create_float_metavar(newface.choices.face_landmarker_score_range))
...
def create_face_selector_program() -> ArgumentParser:
	...
	group_face_selector.add_argument('--face-selector-mode', help = translator.get('help.face_selector_mode'), default = config.get_str_value('face_selector', 'face_selector_mode', 'reference'), choices = newface.choices.face_selector_modes)
	group_face_selector.add_argument('--face-selector-order', help = translator.get('help.face_selector_order'), default = config.get_str_value('face_selector', 'face_selector_order', 'large-small'), choices = newface.choices.face_selector_orders)
	group_face_selector.add_argument('--face-selector-age-start', help = translator.get('help.face_selector_age_start'), type = int, default = config.get_int_value('face_selector', 'face_selector_age_start'), choices = newface.choices.face_selector_age_range, metavar = create_int_metavar(newface.choices.face_selector_age_range))
	group_face_selector.add_argument('--face-selector-age-end', help = translator.get('help.face_selector_age_end'), type = int, default = config.get_int_value('face_selector', 'face_selector_age_end'), choices = newface.choices.face_selector_age_range, metavar = create_int_metavar(newface.choices.face_selector_age_range))
	group_face_selector.add_argument('--face-selector-gender', help = translator.get('help.face_selector_gender'), default = config.get_str_value('face_selector', 'face_selector_gender'), choices = newface.choices.face_selector_genders)
	group_face_selector.add_argument('--face-selector-race', help = translator.get('help.face_selector_race'), default = config.get_str_value('face_selector', 'face_selector_race'), choices = newface.choices.face_selector_races)
	...
	group_face_selector.add_argument('--reference-face-distance', help = translator.get('help.reference_face_distance'), type = float, default = config.get_float_value('face_selector', 'reference_face_distance', '0.3'), choices = newface.choices.reference_face_distance_range, metavar = create_float_metavar(newface.choices.reference_face_distance_range))
...
def create_face_masker_program() -> ArgumentParser:
	...
	group_face_masker.add_argument('--face-occluder-model', help = translator.get('help.face_occluder_model'), default = config.get_str_value('face_masker', 'face_occluder_model', 'xseg_1'), choices = newface.choices.face_occluder_models)
	group_face_masker.add_argument('--face-parser-model', help = translator.get('help.face_parser_model'), default = config.get_str_value('face_masker', 'face_parser_model', 'bisenet_resnet_34'), choices = newface.choices.face_parser_models)
	group_face_masker.add_argument('--face-mask-types', help = translator.get('help.face_mask_types').format(choices = ', '.join(newface.choices.face_mask_types)), default = config.get_str_list('face_masker', 'face_mask_types', 'box'), choices = newface.choices.face_mask_types, nargs = '+', metavar = 'FACE_MASK_TYPES')
	group_face_masker.add_argument('--face-mask-areas', help = translator.get('help.face_mask_areas').format(choices = ', '.join(newface.choices.face_mask_areas)), default = config.get_str_list('face_masker', 'face_mask_areas', ' '.join(newface.choices.face_mask_areas)), choices = newface.choices.face_mask_areas, nargs = '+', metavar = 'FACE_MASK_AREAS')
	group_face_masker.add_argument('--face-mask-regions', help = translator.get('help.face_mask_regions').format(choices = ', '.join(newface.choices.face_mask_regions)), default = config.get_str_list('face_masker', 'face_mask_regions', ' '.join(newface.choices.face_mask_regions)), choices = newface.choices.face_mask_regions, nargs = '+', metavar = 'FACE_MASK_REGIONS')
	group_face_masker.add_argument('--face-mask-blur', help = translator.get('help.face_mask_blur'), type = float, default = config.get_float_value('face_masker', 'face_mask_blur', '0.3'), choices = newface.choices.face_mask_blur_range, metavar = create_float_metavar(newface.choices.face_mask_blur_range))
	group_face_masker.add_argument('--face-mask-padding', help = translator.get('help.face_mask_padding'), type = partial(sanitize_int_range, int_range = newface.choices.face_mask_padding_range), default = config.get_int_list('face_masker', 'face_mask_padding', '0 0 0 0'), nargs = '+')
...
def create_voice_extractor_program() -> ArgumentParser:
	...
	group_voice_extractor.add_argument('--voice-extractor-model', help = translator.get('help.voice_extractor_model'), default = config.get_str_value('voice_extractor', 'voice_extractor_model', 'kim_vocal_2'), choices = newface.choices.voice_extractor_models)
...
def create_frame_extraction_program() -> ArgumentParser:
	...
	group_frame_extraction.add_argument('--trim-frame-start', help = translator.get('help.trim_frame_start'), type = int, default = newface.config.get_int_value('frame_extraction', 'trim_frame_start'))
	group_frame_extraction.add_argument('--trim-frame-end', help = translator.get('help.trim_frame_end'), type = int, default = newface.config.get_int_value('frame_extraction', 'trim_frame_end'))
	group_frame_extraction.add_argument('--temp-frame-format', help = translator.get('help.temp_frame_format'), default = config.get_str_value('frame_extraction', 'temp_frame_format', 'png'), choices = newface.choices.temp_frame_formats)
...
def create_output_creation_program() -> ArgumentParser:
	...
	group_output_creation.add_argument('--output-image-quality', help = translator.get('help.output_image_quality'), type = int, default = config.get_int_value('output_creation', 'output_image_quality', '80'), choices = newface.choices.output_image_quality_range, metavar = create_int_metavar(newface.choices.output_image_quality_range))
	group_output_creation.add_argument('--output-image-scale', help = translator.get('help.output_image_scale'), type = float, default = config.get_float_value('output_creation', 'output_image_scale', '1.0'), choices = newface.choices.output_image_scale_range)
	...
	group_output_creation.add_argument('--output-audio-quality', help = translator.get('help.output_audio_quality'), type = int, default = config.get_int_value('output_creation', 'output_audio_quality', '80'), choices = newface.choices.output_audio_quality_range, metavar = create_int_metavar(newface.choices.output_audio_quality_range))
	group_output_creation.add_argument('--output-audio-volume', help = translator.get('help.output_audio_volume'), type = int, default = config.get_int_value('output_creation', 'output_audio_volume', '100'), choices = newface.choices.output_audio_volume_range, metavar = create_int_metavar(newface.choices.output_audio_volume_range))
	...
	group_output_creation.add_argument('--output-video-preset', help = translator.get('help.output_video_preset'), default = config.get_str_value('output_creation', 'output_video_preset', 'veryfast'), choices = newface.choices.output_video_presets)
	group_output_creation.add_argument('--output-video-quality', help = translator.get('help.output_video_quality'), type = int, default = config.get_int_value('output_creation', 'output_video_quality', '80'), choices = newface.choices.output_video_quality_range, metavar = create_int_metavar(newface.choices.output_video_quality_range))
	group_output_creation.add_argument('--output-video-scale', help = translator.get('help.output_video_scale'), type = float, default = config.get_float_value('output_creation', 'output_video_scale', '1.0'), choices = newface.choices.output_video_scale_range)
...
def create_processors_program() -> ArgumentParser:
	program = ArgumentParser(add_help = False)
	available_processors = [ get_file_name(file_path) for file_path in resolve_file_paths('newface/processors/modules') ]
...
def create_uis_program() -> ArgumentParser:
	program = ArgumentParser(add_help = False)
	available_ui_layouts = [ get_file_name(file_path) for file_path in resolve_file_paths('newface/uis/layouts') ]
	...
	group_uis.add_argument('--ui-workflow', help = translator.get('help.ui_workflow'), default = config.get_str_value('uis', 'ui_workflow', 'instant_runner'), choices = newface.choices.ui_workflows)
	return program
...
def create_download_providers_program() -> ArgumentParser:
	...
	group_download.add_argument('--download-providers', help = translator.get('help.download_providers').format(choices = ', '.join(newface.choices.download_providers)), default = config.get_str_list('download', 'download_providers', ' '.join(newface.choices.download_providers)), choices = newface.choices.download_providers, nargs = '+', metavar = 'DOWNLOAD_PROVIDERS')
...
def create_download_scope_program() -> ArgumentParser:
	program = ArgumentParser(add_help = False)
	group_download = program.add_argument_group('download')
	group_download.add_argument('--download-scope', help = translator.get('help.download_scope'), default = config.get_str_value('download', 'download_scope', 'lite'), choices = newface.choices.download_scopes)
...
def create_benchmark_program() -> ArgumentParser:
	...
	group_benchmark.add_argument('--benchmark-mode', help = translator.get('help.benchmark_mode'), default = config.get_str_value('benchmark', 'benchmark_mode', 'warm'), choices = newface.choices.benchmark_modes)
	group_benchmark.add_argument('--benchmark-resolutions', help = translator.get('help.benchmark_resolutions'), default = config.get_str_list('benchmark', 'benchmark_resolutions', get_first(newface.choices.benchmark_resolutions)), choices = newface.choices.benchmark_resolutions, nargs = '+')
	group_benchmark.add_argument('--benchmark-cycle-count', help = translator.get('help.benchmark_cycle_count'), type = int, default = config.get_int_value('benchmark', 'benchmark_cycle_count', '5'), choices = newface.choices.benchmark_cycle_count_range)
	return program
...
def create_execution_program() -> ArgumentParser:
	...
	group_execution.add_argument('--execution-thread-count', help = translator.get('help.execution_thread_count'), type = int, default = config.get_int_value('execution', 'execution_thread_count', '8'), choices = newface.choices.execution_thread_count_range, metavar = create_int_metavar(newface.choices.execution_thread_count_range))
...
def create_memory_program() -> ArgumentParser:
	...
	group_memory.add_argument('--video-memory-strategy', help = translator.get('help.video_memory_strategy'), default = config.get_str_value('memory', 'video_memory_strategy', 'strict'), choices = newface.choices.video_memory_strategies)
	group_memory.add_argument('--system-memory-limit', help = translator.get('help.system_memory_limit'), type = int, default = config.get_int_value('memory', 'system_memory_limit', '0'), choices = newface.choices.system_memory_limit_range, metavar = create_int_metavar(newface.choices.system_memory_limit_range))
...
def create_log_level_program() -> ArgumentParser:
	...
	group_misc.add_argument('--log-level', help = translator.get('help.log_level'), default = config.get_str_value('misc', 'log_level', 'info'), choices = newface.choices.log_levels)
...
def create_job_status_program() -> ArgumentParser:
	program = ArgumentParser(add_help = False)
	program.add_argument('job_status', help = translator.get('help.job_status'), choices = newface.choices.job_statuses)
	return program
